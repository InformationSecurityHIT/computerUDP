...
import MyUI.RoundBorder;

import javax.swing.*;
import java.awt.*;

public class RoundedBorder extends JButton {
    public RoundedBorder(String s) {
        super(s);
    ...
    protected void paintComponent(Graphics g) {
        if (getModel().isArmed()) {
            g.setColor(new Color(228,84,47));//按下后按钮变成绿色
        } else {
            g.setColor(getBackground());
        }
        g.fillRoundRect(0, 0, getSize().width - 1, getSize().height - 1, 0, 0);//填充圆角矩形边界
        // 这个调用会画一个标签和焦点矩形。
